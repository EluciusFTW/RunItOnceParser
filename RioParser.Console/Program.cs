...
namespace RioParser.Console
{
    class Program
    {
        ...
        static void Main(
            string path = @"C:\Users\bussg\EluciusFTW\RunItOnceParser\Sample\HandHistoryBatch", 
        ...
        private static void GenerateReport(string path, string hero, ReportType reportType, GameType gameType)
        {
            ...
            var reportsGenerator = new ReporterFactory(Logger).Create(reportType);
            var reports = reportsGenerator.Process(handHistoryFiles, hero, gameType);
